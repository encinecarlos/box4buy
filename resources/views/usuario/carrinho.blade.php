								...
								<tr>													
									...
									<td class="col-sm-2">
										<div class="input-group">																
											<input type="text" value="{{ $value['qtde'] }}" id="edita-{{ $es }}" class="form-control qtde-produto">
											<div class="input-group-btn"><button class="btn changeqtd" id="{{ $es }}" type="button" title="Alterar quantidade"><i class="fa fa-edit"></i></button></div>
										</div>															
									</td>														
									<td class="col-sm-1" id="peso_valor">{{ $value['peso'] != '' ? $value['peso'] : '0' }}</td>
									<td id="valor_declarado"><input type="text" name="valor_declarado[]" data-product="{{ $value['id'] }}" id="{{ $es }}" class="form-control" placeholder="Valor unitário"></td>
									<td><a href="#" id="{{ $es }}" class="btn btn-danger removeproduto" data-product="{{ $value['id'] }}" data-qtd="{{ $value['qtde'] }}"><i class="fa fa-close"></i></a></td>
								</tr>										
								@endforeach
							@else
				...
				<div class="form-group">
					...
					<div class="col-sm-10">
						<select name="codigo_endereco" class="form-control select2" style="width: 100%;">
							...
							<option value="{{ $e->seq_endereco }}">{{ $e->endereco }}, {{ $e->numero }} - {{ $e->bairro }} - {{ $e->cidade }} / {{ $e->codigo_postal }}</option>
							@endforeach
						</select>
					</div>
				</div>
							...
							<tr>
								<td colspan="2">
									{{-- <button type="button" id="geraorcamento" class="btn btn-info btn-lg boxColorTema pull-right">
										...
										<a href="#verificadados" class="btn btn-info btn-lg boxColorTema pull-right" id="solicita_orcamento" data-suite="{{ Auth::user()->codigo_suite }}" rel="modal:open">
